...
struct AddressHasher {
	...
	size_t operator() (const Address& address) const {
		...
		return coef*coef * string_hash(address.city)
			+ coef * string_hash(address.street)
			+ address.building;
	}
};
...
struct PersonHasher {
	...
	size_t operator() (const Person& person) const {
		...
		return coef*coef*coef * string_hash(person.name)
			+ coef*coef * person.height
			+ coef * double_hash(person.weight)
